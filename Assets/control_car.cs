...
public class control_car : MonoBehaviour {
	...
	void Start () {
		...
		r.centerOfMass = new Vector3 (0.0f, 0.0f, 0.0f); //need to study this line of code.
	}
	
	// Update is called once per frame
	void FixedUpdate () {
		//add/removes torque if the player presses up or down
		instantPower = Input.GetAxis ("Vertical") * motorPower * Time.deltaTime;
		//
		wheelTurn = Input.GetAxis ("Horizontal") * maxTurn;

		brake = Input.GetKey ("space") ? r.mass * 0.1f : 0.0f;
